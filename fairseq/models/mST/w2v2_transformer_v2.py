...
@register_model("w2v2_transformer_v2")
class W2V2TransformerV2(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        mrasp2_params = checkpoint_utils.load_checkpoint_to_cpu(os.path.join(args.mrasp2_dir, '12e12d_last.pt'))['model']
        mrasp2_encoder_params = {k[8:]: v for k, v in mrasp2_params.items() if k.startswith('encoder.')}
        mrasp2_decoder_params = {k[8:]: v for k, v in mrasp2_params.items() if k.startswith('decoder.')}
        encoder.transformer_encoder.load_state_dict(mrasp2_encoder_params)
        decoder.load_state_dict(mrasp2_decoder_params)
    ...
    def forward_encoder(self, src_tokens, src_lengths, **extra_args):
        encoder_out = self.encoder(src_tokens=src_tokens, src_lengths=src_lengths, **extra_args)
        return encoder_out
...
@register_model_architecture('w2v2_transformer_v2', 'xlsr_mrasp2_base')
def w2v2_transformer_v2_base(args):

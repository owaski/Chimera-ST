...
@register_criterion('multilingual_triplet_align_adv_v2_criterion')
class MultilingualTripletAlignAdvV2Criterion(LabelSmoothedCrossEntropyCriterion):
    def __init__(
        self, 
        task, 
        sentence_avg, 
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.loss_ratio[3] > 0:
            s_encoder_out = model.encoder(**sample["adv_s_input"])
            s_logits = model.discriminator(s_encoder_out.encoder_out, s_encoder_out.encoder_padding_mask)
            s_adv_loss = F.cross_entropy(s_logits, sample["src_lang_indices"], reduction='sum')

            t_encoder_out = model.encoder(**sample["adv_t_input"])
            t_logits = model.discriminator(t_encoder_out.encoder_out, t_encoder_out.encoder_padding_mask)
            t_adv_loss = F.cross_entropy(t_logits, sample["src_lang_indices"], reduction='sum')

            adv_loss = s_adv_loss + t_adv_loss
            adv_sample_size = sample["nsentences"] * 2
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        if adv_sample_size > 0:
            metrics.log_scalar('adv_loss', adv_loss_sum / adv_sample_size, adv_sample_size, round=3)

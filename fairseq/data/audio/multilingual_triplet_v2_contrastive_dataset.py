...
class MultilingualTripletDataset(SpeechToTextDataset):
    ...
    def __getitem__(
        ...
        use_mix = op.exists(self.mixed_audio_paths[index])
        mixed_source = match = None
        ...
        if self.normalize:
            with torch.no_grad():
                ...
                mixed_source = F.layer_norm(mixed_source, mixed_source.shape)
      
        if use_mix:
            mixed_source = get_features_or_waveform(
                self.mixed_audio_paths[index],
                need_waveform=self.data_cfg.use_audio_input,
                sample_rate=self.sample_rate,
            )
            if self.feature_transforms is not None:
                assert not self.data_cfg.use_audio_input
                mixed_source = self.feature_transforms(mixed_source)
            mixed_source = torch.from_numpy(mixed_source).float()
            if self.normalize:
                with torch.no_grad():
                    mixed_source = F.layer_norm(mixed_source, mixed_source.shape)
            match = torch.load(self.match_paths[index])        
        else:
            flag = True
        ...
        return index, source, mixed_source, match, target, src_text, src_lang_tag_idx
...
class MultilingualTripletDatasetCreator(SpeechToTextDatasetCreator):
    ...
    def _from_list(
        ...
        if len(parts) == 1: # unlabeled data
            ...
            labeled = True
            audio_paths, mixed_audio_paths, match_paths, n_frames, src_texts, tgt_texts, ids = [], [], [], [], [], [], []
            ...
            for s in samples:
                ...
                mixed_audio_paths.extend(
                    [op.join(data_cfg.audio_root, ss[cls.KEY_SRC_LANG], '16kHz', ss[cls.KEY_ID] + '-mixed.wav') for ss in s]
                )
                match_paths.extend(

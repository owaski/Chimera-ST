...
class MultilingualTripletContrastiveDataset(SpeechToTextDataset):
    ...
    def __getitem__(
        ...
        if self.ref_audio_paths is not None:
            ref = self.ref_audio_paths[index]
            if len(ref) > 0:
                choice = np.random.randint(len(ref))
...
class MultilingualTripletContrastiveDatasetCreator(SpeechToTextDatasetCreator):
    ...
    def _from_list(
        ...
        if len(parts) == 1: # unlabeled data
            ...
            ref_audio_paths = []
            for s, r in zip(samples, ref_audios):
                ids.extend([ss[cls.KEY_ID] for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0])
                audio_paths.extend(
                    [op.join(data_cfg.audio_root, ss[cls.KEY_SRC_LANG], '16kHz', ss[cls.KEY_AUDIO]) for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0]
                )
                n_frames.extend([int(ss[cls.KEY_N_FRAMES]) for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0])
                tgt_texts.extend([ss[cls.KEY_TGT_TEXT] for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0])
                src_texts.extend(
                    [ss.get(cls.KEY_SRC_TEXT, cls.DEFAULT_SRC_TEXT) for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0]
                )
                speakers.extend([ss.get(cls.KEY_SPEAKER, cls.DEFAULT_SPEAKER)
                                for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0])
                src_langs.extend([ss.get(cls.KEY_SRC_LANG, cls.DEFAULT_LANG)
                                for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0])
                tgt_langs.extend([ss.get(cls.KEY_TGT_LANG, cls.DEFAULT_LANG)
                                for idx, ss in enumerate(s) if r is None or len(r[idx]) > 0])
                if r is not None:
                    ref_audio_paths.extend([rr for rr in r if len(rr) > 0])
        ...
        return MultilingualTripletContrastiveDataset(
            ...
            labeled=labeled,
            ref_audio_paths=None if len(ref_audio_paths) == 0 else ref_audio_paths
        )
    ...
    def from_tsv(
        ...
        for split in _splits:
            ...
            if len(parts) == 1: # unlabelled data
                ...
                ref_path = op.join(root, src_code, "{}_refs.pt".format(tp))
                types[1] = True
            ...
            if op.exists(ref_path):
                ref_audio = torch.load(ref_path)
        ...
        datasets = [
            ...
            for name, s, r in zip(_splits, samples, ref_audios)
        ]

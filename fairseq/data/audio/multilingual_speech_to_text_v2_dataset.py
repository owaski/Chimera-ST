...
from fairseq.data.audio.audio_utils import \
    get_fbank, get_waveform, get_waveform_chi
...
class MultilingualS2TDataConfig(object):
...
def get_features_or_waveform(path: str, need_waveform=False, sample_rate=16000):
    ...
    if len(extra) == 0:
        if need_waveform:
            wave, ori_sr = get_waveform(_path)
            if ori_sr != sample_rate:
                wave = wave[::(ori_sr // sample_rate)]
            return wave
        ...
        if _path.endswith('.zip'):
            ...
            assert need_waveform, f"path {_path} and not need_waveform conflict"
            wave, ori_sr = get_waveform_chi(_path, int(extra[0]), int(extra[1]))
            if ori_sr != sample_rate:
                wave = wave[::(ori_sr // sample_rate)]
            return wave
    else:
...
class MultilingualSpeechToTextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        split: str,
        is_train_split: bool,
        data_cfg: MultilingualS2TDataConfig,
    ...
    def __getitem__(
        ...
        if self.src_langs is not None and self.data_cfg.prepend_src_lang_tag:
            lang_tag = self.LANG_TAG_TEMPLATE.format(self.lang2langcode[self.src_langs[index][:2]])
            src_lang_tag_idx = self.src_dict.index(lang_tag)

        return index, source, target, src_lang_tag_idx
    ...
    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor]]) -> Dict:
        ...
        src_lang_tag_indices = torch.tensor(
            [src_lang_tag_idx for _, _, _, src_lang_tag_idx in samples], dtype=torch.long
...
class MultilingualSpeechToTextDatasetCreator(object):
    # mandatory columns
    ...
    def _from_list(
        ...
        return MultilingualSpeechToTextDataset(
            split_name,
            is_train_split,
            data_cfg,
            audio_paths,

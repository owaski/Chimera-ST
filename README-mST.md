...
Training:
```bash
export EXP_ID="xlsr_mbart_de_zh_mixed"
# export EXP_ID="debug"
...
fairseq-train $COVOST2_ROOT \
  ...
  --max-tokens 400000 --max-tokens-valid 3200000 --max-source-positions 800000 \
  ...
  --lr 5e-5 --lr-scheduler inverse_sqrt --weight-decay 0.0 --dropout 0.1 \
...
CUDA_VISIBLE_DEVICES=6 fairseq-generate ${COVOST2_ROOT} --gen-subset zh-CN_en_test \
  --task multilingual_speech_to_text --path /mnt/data/siqiouyang/runs/mST/xlsr_mbart_de_zh_contrastive/checkpoint_best.pt \
